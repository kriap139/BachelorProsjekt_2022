...
class PreStateDetectProcess(mp.Process):
    def __init__(self, mainExitEvent: mp.Event(), maxTimeoutSec=10):
        ...
        self.stateDProc: StateDetectProcess = None
    ...
    def run(self) -> None:
        ...
        self.stateDProc = StateDetectProcess(self.mainExitEvent, self.preSDetectQueue, self.postSDetectQueue, self.maxTimeoutSec)
        self.stateDProc.start()
        self.stateDProc.activate()
        ...
        while data.dfsActive:
            ...
            if len(bboxes) > 0:
                ...
                if len(tagBoxes) > 0:
                    ...
                    imageData = ImageData(data.args.streamID, SharedImage(shm.name, copy.dtype, copy.shape), bboxes)
                    shm.close()

                    self.preSDetectQueue.put(imageData)
